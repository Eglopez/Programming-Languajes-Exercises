...
class Token:
    ...
    def info(self):
        return(
            "".join(list(map(lambda x: chr(x),self.value))),self.type
        )
...
class Automata:
    ...
    def tokenCreator(self,text,i,token=None):
        ...
        if not token.inFormation and self.isQuote(char):
            ...
            if self.isQuote(token.atFirst()) and not self.isQuote(char):
                token.add(char)
            else:
                token.add(char)
                token.formed = True
